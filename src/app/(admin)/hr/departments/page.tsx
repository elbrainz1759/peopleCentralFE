            ...
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
                ...
                <div className="block md:hidden min-h-[400px]">
                    {isLoading ? (
                        ...
                        <p className="py-10 text-center text-gray-500">No departments found.</p>
                    ) : (
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            {filtered.map((dept) => (
                                <div key={dept.id} className="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                                    <div className="flex items-start justify-between mb-2">
                                        <p className="font-medium text-gray-800 dark:text-white/90 text-sm">{dept.name}</p>
                                    ...
                                    <div className="space-y-1 text-xs text-gray-500 dark:text-gray-400 mb-3">
                                        <div className="flex justify-between">
                                            ...
                                            <span>{dept.created_by || "System"}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="font-medium text-gray-600 dark:text-gray-300">Registration Date</span>
                                            <span>{dept.created_at ? new Date(dept.created_at).toLocaleDateString() : "Internal"}</span>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => openEdit(dept)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800">
                                            <PencilIcon className="w-3.5 h-3.5" /> Edit
                                        </button>
                                        <button onClick={() => handleDelete(dept)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-red-100 text-xs font-medium text-red-500 hover:bg-red-50 dark:border-red-900/30 dark:hover:bg-red-900/10">
                                            <TrashBinIcon className="w-3.5 h-3.5" />
                                            {isDeleting === dept.unique_id ? "Removing..." : "Remove"}
                                        </button>
                                    </div>
                                </div>
                            ))}
                    ...
                    <Table>
                        <TableHeader className="border-gray-100 dark:border-gray-800 border-y">
                            <TableRow>
                                ...
                                <TableCell isHeader className="py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Department Name</TableCell>
                        ...
                        <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {isLoading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="py-10 text-center text-gray-500">No departments found.</TableCell>
                                </TableRow>
                            ) : (
                                filtered.map((dept, index) => (
                                    <TableRow key={dept.id}>
                                        <TableCell className="py-3 text-theme-sm text-gray-500">{index + 1}</TableCell>
                                        <TableCell className="py-3 font-medium text-gray-800 dark:text-white/90">{dept.name}</TableCell>
                                        <TableCell className="py-3 text-theme-sm text-gray-500">{dept.created_by || "System"}</TableCell>
                                        <TableCell className="py-3 text-theme-sm text-gray-500">{dept.created_at ? new Date(dept.created_at).toLocaleDateString() : "Internal"}</TableCell>
                                        ...
                                        <TableCell className="py-3 relative">
                                            <button className="dropdown-toggle flex items-center justify-center h-8 w-8 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" onClick={() => setActiveDropdown(activeDropdown === dept.id ? null : dept.id)}>
                                                <HorizontaLDots className="h-4 w-4 text-gray-500" />
                                            </button>
                                            <Dropdown isOpen={activeDropdown === dept.id} onClose={() => setActiveDropdown(null)} className="absolute right-0 top-10 pointer-events-auto">
                                                <div className="w-48 py-2">
                                                    ...
                                                    <DropdownItem onClick={() => { setActiveDropdown(null); handleDelete(dept); }} className="text-red-500 hover:text-red-600 hover:bg-red-50">
                                                        <div className="flex items-center gap-2"><TrashBinIcon className="w-4 h-4" /><span>{isDeleting === dept.unique_id ? "Removing..." : "Remove Record"}</span></div>
                                                    </DropdownItem>
                                                </div>
                                            </Dropdown>
                                        </TableCell>
